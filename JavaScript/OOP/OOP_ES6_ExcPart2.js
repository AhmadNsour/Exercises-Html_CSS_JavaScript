...
const sumOddNum = (numbers)=>{
  var arr = [];
  arr=numbers.filter(item=>{
    return item%2 !=0 ;
  })
return arr;
}
//Excersice 2: Write a JavaScript program to reverse the order of the characters in the string.
const reverseChar = (character)=>{
  Str = character.split('').reverse().join('');
return	Str; 
}
...
*/
const capitalizeChar = (character)=>{
  Str = character.split('').toLowerCase()
return	Str; 
}
